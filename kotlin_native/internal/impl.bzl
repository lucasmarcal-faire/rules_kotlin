...
def kt_native_library_impl(ctx):
    toolchain = ctx.toolchains[_TOOLCHAIN_TYPE]

    libraries = depset([d[_KtNativeInfo].jar for d in ctx.attr.deps])
    ...
    ctx.actions.run(
        ...
        arguments = [args],
        progress_message = "Compiling Kotlin Native %%{label} { kt: %d }" % len(ctx.files.srcs),
